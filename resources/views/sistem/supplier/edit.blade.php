...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            {!! Form::model($supplier, ['method' => 'post', 'route' => ['sistem.supplier.update', $supplier->id], 'enctype' => 'multipart/form-data']) !!}
                <div class="form-group mb-3">
                    <label for="" class="form-label">Nama Supplier</label>
                    <input type="text" name="nama_supplier" class="form-control" value="{{ $supplier->nama_suppliers }}">
                    <i class="text-danger">{{ $errors->first('nama_supplier') }}</i>
                </div>
                <div class="form-group mb-3">
                    <label for="" class="form-label">Toko Supplier</label>
                    <input type="text" name="toko_supplier" class="form-control" value="{{ $supplier->toko_suppliers }}">
