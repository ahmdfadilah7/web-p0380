...
class CartController extends Controller
{
    ...
    public function index()
    {
        ...
        $transaksi = Transaksi::join('barangs', 'transaksis.barang_id', 'barangs.id')
            ...
            ->select('transaksis.*', 'barangs.nama_barang', 'barangs.diskon', 'barangs.harga_diskon', 'barangs.foto_barang', 'barangs.harga_barang', 'invoices.kode_invoice')
            ->get();
        
        $invoice = Invoice::where('pelanggan_id', Auth::guard('webpelanggan')->user()->id)
            ->where('status', '0')
            ->first();
        ...
        return view('cart.index', compact('setting', 'kategori', 'transaksi', 'transaksiCount', 'invoice'));
    }
    ...
    public function jasapengiriman($id)
    {
        $jasapengiriman = JasaKirim::find($id);

        return json_encode($jasapengiriman);
    }
    ...
    public function proses_checkout(Request $request, $kode_invoice)
    {
        ...
        $invoice->status = '1';
        $invoice->ongkos_kirim = $request->get('ongkos_kirim');
    ...
    public function invoice_pembayaran($kode_invoice)
    {
        ...
        $transaksi = Transaksi::join('barangs', 'transaksis.barang_id', 'barangs.id')
                ...
                ->select('transaksis.*', 'barangs.nama_barang', 'barangs.diskon', 'barangs.harga_diskon', 'barangs.harga_barang')
                ->get();
    ...
    public function update_kuantitas(Request $request, $id)
    {
        ...
        if ($request->get('kuantitas') > 0) {
            ...
            if ($request->get('kuantitas') > $barang->stok) {
                return redirect()->route('keranjang')->with('warning', 'Stok barang tidak cukup.');
            } else {                
