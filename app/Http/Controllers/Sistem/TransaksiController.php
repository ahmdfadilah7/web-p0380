...
class TransaksiController extends Controller
{
    ...
    // proses menampilkan data transaksi dengan datatables
        ...
        $datatables = DataTables::of($data)
            ...
            ->addColumn('status', function($row) {
                if ($row->status==1) {
                    ...
                    if ($row->konfirmasi==0) {
                        $status = '<span class="badge bg-primary">Menunggu Konfirmasi Administrator</span>';
                    } else {
            ...
            ->addColumn('total_invoice', function($row) {
                $total = AllHelper::rupiah($row->total_invoice);
                return $total;
            })
            ->addColumn('action', function($row) {
                if (Auth::guard('websistem')->user()->role=='Pegawai') {
                    ...
                    if ($row->status==3 && $row->konfirmasi==0) {
                        $btn = '<a href="'.route('sistem.transaksi.konfirmasiAdmin', $row->id).'" class="btn btn-success btn-sm" title="Konfirmasi" style="margin-right: 5px;">
                                    <i class="ti ti-check"></i>
                                </a>';
                        $btn .= '<a href="'.route('sistem.transaksi.batalkan', $row->id).'" class="btn btn-danger btn-sm" title="Batalkan" style="margin-right: 5px;">
                                    <i class="ti ti-x"></i>
    ...
    {
        $transaksi = Transaksi::where('invoice_id', $id)->get();
        foreach ($transaksi as $key => $value) {
            $result = Barang::find($value->barang_id);
            $result->stok = $result->stok - $value->kuantitas;
            $result->save();
        }
    ...
    // Proses Konfirmasi
    public function konfirmasi_admin($id)
    {        
        $invoice = Invoice::find($id);
